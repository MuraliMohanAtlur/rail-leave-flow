...
export async function createApp() {
  ...
  app.post("/api/admin/import-employees", requireAuth, async (req, res) => {
    try {
  ...
  app.get("/api/dashboard/on-leave-today", requireAuth, onLeaveToday);
  app.get("/api/dashboard/on-leave", requireAuth, async (req, res) => {
  ...
  app.get("/api/dashboard/sick-leaves", requireAuth, async (req, res) => {
    res.set("Cache-Control", "no-store");
    const date = String(req.query.date ?? "").trim() || undefined;
    try {
  ...
  app.get("/api/dashboard/stats", requireAuth, dashboardStats);
...
function mountRoutes(app, prefix) {
  app.use(`${prefix}/stations`, requireAuth, stationRoutes);
  app.use(`${prefix}/employees`, requireAuth, employeeRoutes);
  app.use(`${prefix}/leave`, requireAuth, leaveRoutes);
  app.use(`${prefix}/reports`, requireAuth, reportRoutes);

  // Compatibility for existing frontend code while pages are migrated.
  app.use(`${prefix}/leave-requests`, requireAuth, leaveRoutes);
}
